...
**Winery Admin UI** is a UI application for administering a wine store. The application is implemented as a **thin client**, which does not contain business logic for data persistence and interacts with backend services exclusively via HTTP API.
...
> ❗ The UI application **does not use JPA, Hibernate, or direct database access**. All data interactions happen via backend APIs.
...
The entire stack (backend, database, UI, frontend, Prometheus, Grafana, Loki, Tempo) can be launched via **Docker Compose**:
```
